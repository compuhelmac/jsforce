...
Job.prototype.open = function(callback) {
  ...
  var contentType = this.options && this.options && this.options.contentType ? this.options.contentType : 'CSV'
  var headers = {
    "Content-Type": "application/xml; charset=utf-8"
  }

  if (this.options && this.options && this.options.pkChunking) {
    headers['Sforce-Enable-PKChunking'] = this.options.pkChunkingSize ? 'chunkSize=' + this.options.pkChunkingSize : 'true';
  }
 ...
 * @returns {Bulk~Batch}
 */
Job.prototype.createBatch = function(options) {
  var batch = new Batch(this, null, options);
  var self = this;
 ...
 * @returns {Bulk~Batch}
 */
Job.prototype.batch = function(batchId, options) {
  var batch = this._batches[batchId];
  if (!batch) {
    batch = new Batch(this, batchId, options);
...
Job.prototype.check = function(callback) {
  ...
  var bulk = this._bulk;
  var logger = bulk._logger;
  var responseType = self.options.headers && self.options.headers['Content-Type'] ? self.options.headers['Content-Type'] : "application/xml"
 ...
 */
var Batch = function(job, batchId, options) {
...
Batch.prototype._setupDataStreams = function() {
  ...
  this._downloadDataStream = this._downloadStream.stream('csv', converterOptions, batch.options.dataStreamOptions);
...
Batch.prototype._createRequestStream = function() {
  ...
  var logger = bulk._logger;
  var headers = batch.options.headers || {
  ...
  return bulk._request({
    ...
    headers: headers,
    responseType: batch.options.headers && batch.options.headers['Content-Type'] ? batch.options.headers['Content-Type'] : "application/xml"
  }, function(err, res) {
    if (err) {
      batch.emit('error', err);
    } else {
      if (batch.options.headers && batch.options.headers['Content-Type'] === 'application/json') {
        res = { batchInfo: res };
      }
...
Batch.prototype.run =
  Batch.prototype.exec =
    Batch.prototype.execute = function(input, callback) {
      ...
      this.once('notprocessed', function(res) {
        rdeferred.resolve(res.jobId);
      });
...
Batch.prototype.check = function(callback) {
  ...
  return bulk._request({
    ...
    responseType: self.options.headers && self.options.headers['Content-Type'] ? self.options.headers['Content-Type'] : "application/xml"
  }).then(function(res) {
    if (self.options.headers && self.options.headers['Content-Type'] === 'application/json') {
      res = { batchInfo: res };
    }
    logger.debug(res.batchInfo);
...
Batch.prototype.result = function(resultId) {
  ...
  var resultDataStream = resultStream.stream('csv', null, this.options.dataStreamOptions);
 ...
 * @returns {Bulk~Batch}
 */
Bulk.prototype.load = function(type, operation, options, input, callback, extra) {
  var self = this;
  if (!type || !operation) {
...
Bulk.prototype.query = function(soql, options) {
  ...
  var dataStream = recordStream.stream('csv', null, options.dataStreamOptions);
  this.load(type, "query", soql, null, null, options).then(function(results) {
    var streams = results.map(function(result) {
      return self
        .job(result.jobId, options)
        .batch(result.batchId, options)
        .result(result.id)
        .stream();
    });
 ...
 * @returns {RecordStream.Parsable} - Record stream, convertible to CSV data stream
 */
Bulk.prototype.getJob = function(jobId, batchId, options) {
  var self = this;
  var job = self.job(jobId, options);
  var batch = job.batch(batchId, options);
 ...
 * @returns {RecordStream.Parsable} - Record stream, convertible to CSV data stream
 */
Bulk.prototype.getJobResult = function(jobId, batchId, resultId, options) {
  var self = this;
  var job = self.job(jobId, options);
  var batch = job.batch(batchId, options);
  ...
  batch.on('response', function(results) {
    ...
    let resultStream = self
      .job(result.jobId, options)
      .batch(result.batchId, options)
      .result(result.id)
      .stream();

    resultStream.pipe(dataStream);
  });
  batch.on('error', function(err) {
    recordStream.emit('error', err);
  });
 ...
 * @returns {Bulk~Job}
 */
Bulk.prototype.job = function(jobId, options) {
  return new Job(this, null, null, options, jobId);
};

/**
 * Create a bulk Job and return his batch list
 *
 * @param {String} soql - SOQL to execute in bulk job
 * @returns {Promise.<Array.<Bulk~BatchInfo>>}
 */
Bulk.prototype.createBulkJob = function(soql, options) {
  var bulk = this;
  return new Promise(function(resolve, reject) {
    let m = soql.replace(/\([\s\S]+\)/g, '').match(/FROM\s+(\w+)/i);
    if (!m) {
      throw new Error('No sobject type found in query, maybe caused by invalid SOQL.');
    }

    let type = m[1];
    bulk.load(type, 'query', soql, null, null, options).then(function(jobId) {
      bulk
        .job(jobId)
        .list()
...
/**
 * Retrieve a bulk Job Batch
 *
 * @param {Bulk~batchInfo} batch Information
 * @returns {Promise.<RecordStream>}
 */
Bulk.prototype.retrieveBulkJob = function(batchInfo) {
  var bulk = this;
  return new Promise(function(resolve, reject) {
    let batch = bulk
      .job(batchInfo.jobId)
    ...
    batch.on('response', function(results) {
      ...
      resolve(batch.result(result.id).stream());
    });

    batch.poll(bulk.pollInterval, bulk.pollTimeout)
  });
};
